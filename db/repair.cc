    ...
    for (const auto& cf_id_and_tables : cf_id_to_tables) {
      ...
      for (const auto* table : cf_id_and_tables.second) {
        edit.AddFile(
            ...
            table->meta.num_entries,
            table->meta.stats.num_reads_sampled.load(std::memory_order_relaxed),
            table->meta.stats.num_tps_sampled.load(std::memory_order_relaxed),
            table->meta.prefetch_bpk);  // modified by modular filters
      }
